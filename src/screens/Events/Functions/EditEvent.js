    ...
    _updateSelect = (id,name) =>{
      ...
      getEventsCategory = () => {
        ...
        .then(async(responseJson) => {
            const data = responseJson.filter((item) => item.id === this.state.category_id)
            .map((value, index) => {
                this.setState({category_name: value.name})
            })
      ...
      saveEvents = () => {
        ...
        fetch('https://solucardz.com/api/v1/update_events', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        ...
        .then(async(responseJson) => {
          ...
          if(responseJson.error == false){
              ...
              Toast.show('Update Failed, Please check your information');
            }
        })
      }
      ...
      componentDidMount(){
        ...
        this.setState({
            ...
            photo: {uri: 'https://solucardz.com/assets/images/Events/'+events.photo}
        })
      }
    render(){
        if(!this.props.navigation.getParam('events')){
            return <ActivityIndicator size="large"/>
        }
        const { show, date, mode, date_status, time_status, value } = this.state;
        var radio_props = [
        ...
        return(
          ...
          <SafeAreaView style={styles.container}>
            ...
            />
            <Category selected={this.state.category_id} item={this.state.categories} updateSelect={this._updateSelect}  modal={this.state.modalVisible}/> 
          </SafeAreaView>
          </KeyboardAvoidingView>
        )
    }
